...
(function() {
  ...
  */
  var cabinet, chorus, connect, drive, merger, phaser, tuna, wahwah;
  ...
  connect = function(...nodes) {
    ...
    for (i = j = 0, len = nodes.length; j < len; i = ++j) {
      node = nodes[i];
      if (next = nodes[i + 1]) {
        node.connect((ref = (ref1 = next.input) != null ? ref1 : next.destination) != null ? ref : next);
      }
    }
  };

  // # # # # # # # # # # #
  this.pre = actx.createGain();

  pre.gain.value = 0.2; // guitar volume

  this.post = actx.createGain();
