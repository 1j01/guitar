...
pre.connect(convolver.input);
convolver.connect(drive);
drive.connect(post);
...
var GuitarString = function(notestr){
	...
	this.play = function(fret){
		...
		osc.frequency.value = getFrequency(noten);
...
var fretboard = {
	...
	draw: function(){
		...
		for(var x=0, xp=0, fret = 1; fret < this.num_frets; fret++){
			...
			if(!mouseOpen && mouseX-this.x < x && mouseX-this.x >= xp){
				mouseFret = fret;
				mouseFretX = x;
				mouseFretW = xp-x;
			}
		...
		var mouseString = Math.floor((mouseY-this.y)/sh);
		var mouseStringY = (mouseString) * sh;
		for(var s=0;s<this.strings.length;s++){
			...
			if(mouseOverFB && s==mouseString){
				ctx.lineWidth = (s/3+1)*2;
				ctx.strokeStyle = "rgba(150,255,0,0.8)";
				ctx.beginPath();
				ctx.moveTo(mouseFretX,sy);
				ctx.lineTo(this.w,sy);
				ctx.stroke()
			}
		...
		if(mouseOverFB && mouseString>=0 && mouseString<this.strings.length){
			if(mouseDown){
				...
				var noten = this.strings[mouseString].play(mouseFret);
				if(!recNote
				|| recNote.f != mouseFret
			...
			var b = 5;
			ctx.fillRect(mouseFretX+b,mouseStringY+b,mouseFretW/*-b*2*/,sh-b-b);
		...
		for(var i in playingNotes){
			...
			ctx.lineWidth = (note.s/3+1)*2;
			ctx.strokeStyle = "rgba(0,255,255,0.8)";
			ctx.beginPath();
			ctx.moveTo(fretXs[note.f],sy);
			ctx.lineTo(this.w,sy);
			ctx.stroke()
		}
...
$$.on("keydown", function(e){
	...
	if(key === 27){//Escape
		...
		var iid = setInterval(function(){
			fretboard.strings[playNote.s].play(playNote.f);
		});
		$$.on("keyup",function(e){
			if(e.keyCode == key){
